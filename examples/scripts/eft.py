...
def image_outputs_logger(image_data, global_step, accelerate_logger):
    ...
    log_images = []
    for image_tensor, prompt, reward in zip(images, prompts, rewards):
        image = to_pil_image(image_tensor)
        buffer = io.BytesIO()
        image.save(buffer, format='JPEG', quality=75)
        buffer.seek(0)
        log_images.append(wandb.Image(Image.open(buffer), file_type="jpg", caption=f"Prompt: [{prompt}]; Reward: [{reward:.2f}]"))
